...
import { tenantnavbarLinks } from "@/constants";
...
export const TenantSidebar = forwardRef(({ collapsed }, ref) => {
  return (
    <aside
      ref={ref}
      className={cn(
    ...
    >
      ...
      <div className="flex w-full flex-col gap-y-4 overflow-y-auto overflow-x-hidden p-3 [scrollbar-width:_thin]">
        {tenantnavbarLinks.map((tenantnavbarlink) => (
          <nav
            key={tenantnavbarlink.title}
            className={cn("sidebar-group", collapsed && "md:items-center")}
          >
            <p
              className={cn("sidebar-group-title", collapsed && "md:w-[45px]")}
            >
              {tenantnavbarlink.title}
            </p>
            {tenantnavbarlink.links.map((link) => (
              <NavLink
                key={link.label}
                to={link.path}
