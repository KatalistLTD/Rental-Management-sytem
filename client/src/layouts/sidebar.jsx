...
import { landlordnavbarLinks } from "@/constants";
  ...
  return (
    ...
    >
      <div className="flex gap-x-3 p-3">
        <img src={logoLight} alt="Logoipsum" className="dark:hidden max-h-35" />
      ...
      <div className="flex w-full flex-col gap-y-4 overflow-y-auto overflow-x-hidden p-3 [scrollbar-width:_thin]">
        {landlordnavbarLinks.map((landlordnavbarLink) => (
          <nav
            key={landlordnavbarLink.title}
            className={cn("sidebar-group", collapsed && "md:items-center")}
          >
            <p
              className={cn("sidebar-group-title", collapsed && "md:w-[45px]")}
            >
              {landlordnavbarLink.title}
            </p>
            {landlordnavbarLink.links.map((link) => (
              <NavLink
                key={link.label}
                to={link.path}
